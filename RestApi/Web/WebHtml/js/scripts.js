...
$(document).ready(function() {
    ...
    ko.applyBindings(studentEditor, $("#student-add")[0]);
...
function Update(controllerName, method, vm, id) {
    ...
    $.ajax({
        ...
        contentType: "application/json",
        success: function(data) {
            alert("Aktualizacja danych się powiodła.");
        },
        error: function(error) {
            var errorVM = new ErrorViewModel();
            var obj = JSON.parse(error.responseText);
...
function DeleteStudent(index) {
    var respond = confirm("Czy na pewno chcesz usnunąć?");
    if (respond) {
...
function DeleteCourse(index) {
    var respond = confirm("Czy na pewno chcesz usnunąć ten przedmiot?");
    if (respond) {
